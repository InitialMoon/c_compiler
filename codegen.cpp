...
map<string, int> Variable_Address;// 记录所有的变量名，以及其对应的地址
set<function_info> function_info_map;
...
void analysis_function_call(ASTNode *node) {
    ...
    if (node->data.binary.left) {
        translateAST(node->data.binary.left);
    }
    if (node->data.binary.right) {
        translateAST(node->data.binary.right);
    }
}
...
void analysis_function_definition(ASTNode *node) {
    ...
    function_info_map.insert(new_function);
}
...
void analysis_function_list (ASTNode *node) {
    ...
    switch (node->type) {
        ...
        case NODE_FUNCTION_DEFINITION:
            ...
            break;
        default:
            printf("Unknown node type in function list: %d\n");
            break;
    }
}
...
void translateAST(ASTNode *node) {
    ...
    switch (node->type) {
        case NODE_TRANSLATION_UNIT:
            ...
            break;
        default:
            printf("Unknown node type at root: %d\n");
            break;
    }
}
...
void print_global_function() {
	printf(".global main\n");
    for(auto it = function_info_map.begin(); it != function_info_map.end(); it++) {
